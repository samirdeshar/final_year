  ...
  <section class="section">
    <div class="row">
      <div class="col-lg-12">
        ...
        <div class="card">
          <div class="card-body">
            <h5 class="card-title" style="float">Video List</h5>
            <a href="{{ route('video.create')}}" style="float:right;margin-right:20px;" class="btn btn-sm btn-success"><i class="bi bi-plus"></i>Create New Video</a>

            <p>All Video Listed Here.....</p>
              ...
              <!-- Example single danger button -->
                <div class="btn-group" style="float:left">
                  ...
                  <ul class="dropdown-menu bg-success ">
                    <li><a class="dropdown-item text-white drop-hover" href="{{ route('get-video-status','active')}}"><i class="bi bi-check-circle me-1"></i> Active</a></li>
                    <li><a class="dropdown-item text-white drop-hover" href="{{ route('get-video-status','inactive')}}"><i class="bi bi-exclamation-octagon me-1"></i> In-Active</a></li>
                    <li><a class="dropdown-item text-white drop-hover" href="{{ route('get-video-status','all')}}"><i class="bi bi-collection-fill me-1"></i> All</a></li>
                  </ul>
                </div>
              {{-- -------------------Status Drop Down----------------- --}}
            ...
            <form action="{{ route('to-video')}}" method="post" id="full-form">
                ...
                {{-- @dd($data) --}}
                    @foreach($data as $post)
                        <tr>
                            ...
                            <td>
                                @if($post->video_url && $post->video_url !=null)
                                    {{-- <a href="{{ asset(@$post->video_url)}}" data-lightbox="image-{{$post->id}}" data-title="{{ ucfirst($post->title)}}">
                                        View
                                    </a> --}}
                            ...
                            <td>
                                <a href="{{ route('video-update-status',[$post->id,$post->status])}}">
                            ...
                            <td>
                                <a href="{{ route('video.edit',$post)}}" class="btn btn-sm btn-primary btn-style">
                                    <i class="bi bi-pen-fill"></i>
                                </a>
                                <a href="javascript:;" data-id="{{ $post->id }}" class="btn btn-sm btn-danger btn-style delete-video">
...
<script>
                ...
                $(document).on('click', '.delete-video', function(e) {
                   e.preventDefault();
                    var myAction = "{{route('video.destroy', ':id')}}"
                    var productId = $(this).data('id');
                    myAction = myAction.replace(':id', productId);
                    $('.delete-form').attr('action', myAction);
                    let clicked = confirm('Are You Sure Want To Delete Video !');
